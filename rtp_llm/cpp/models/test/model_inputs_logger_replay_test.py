...
def _iter_model_inputs_dumps(path: Path) -> list[Path]:
    if path.is_file():
        return [path]
    return sorted(path.rglob("*.pt"), key=lambda p: str(p.relative_to(path)))
...
class ModelInputsLoggerReplayTest(unittest.TestCase):
    def test_generated_dump_can_be_loaded_by_torch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            dump_dir = Path(temp_dir) / "model_inputs_r2_s3"
            dumps = sorted(dump_dir.glob("model_inputs_r2_s3_*.pt"))
